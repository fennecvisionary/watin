...
import { 
  ...
  MapPinned, Syringe, FileText, CheckCircle, Clock
} from 'lucide-react';
...
function PreviewCard({ name, blood, wilaya, commune, phone, type, lastDon, isAnonymous, contactTime }: {
  name: string;
  blood: string;
  wilaya: string;
  commune: string;
  ...
  const displayName = isAnonymous ? 'فاعل خير' : (name || 'اسمك هنا');
    ...
    <div className="preview-card-wrapper">
      ...
      <div className="donor-card-pro preview-card-actual">
        ...
        <div className="donor-chips preview-chips">
          ...
          {lastDon && (
            <span className="donor-chip" style={{ background: '#f1f5f9', color: '#475569', width: '100%', justifyContent: 'flex-start', paddingRight: '12px', gap: '8px' }}>
              <Calendar size={12} />
              <span>آخر تبرع: منذ {Math.floor((new Date().getTime() - new Date(lastDon).getTime()) / (1000 * 60 * 60 * 24))} يوم</span>
            </span>
          )}
          {contactTime && contactTime !== 'في أي وقت (الافتراضي)' && (
            <span className="donor-chip" style={{ background: '#f1f5f9', color: '#475569', width: '100%', justifyContent: 'flex-start', paddingRight: '12px', gap: '8px' }}>
              <Clock size={12} />
              <span>يفضل الاتصال: {contactTime}</span>
            </span>
          )}
        </div>
        ...
        <div className="donor-actions preview-actions">
          <a className="donor-call-btn preview-call-btn" style={{ pointerEvents: 'none', padding: '8px 16px', minHeight: '40px' }}>
            <span className="call-number" dir="ltr" style={{ direction: 'ltr', unicodeBidi: 'embed', fontSize: '1rem' }}>{displayPhone}</span>
...
export default function Register({ onRegister }: { onRegister: (donor: any) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalName = isAnonymous ? 'فاعل خير' : name;
    const nameWithContactTime = contactTime ? `${finalName} (${contactTime})` : finalName;

    const newDonorData = {
      name: nameWithContactTime,
      phone: phone.replace(/\s/g, ''),
      wilaya: selectedWilaya,
      commune,
      blood,
      type: type as "blood" | "both",
      lastDon: lastDon ? new Date(lastDon) : new Date()
    };

    // Import from '../data' handled dynamically
    const { addDonorToSupabase } = await import('../data');
    const success = await addDonorToSupabase(newDonorData);

    setIsSubmitting(false);

    if (success) {
      onRegister(newDonorData);
      setIsSubmitted(true);
                ...
                <form id="reg-form" onSubmit={handleSubmit}>
                  <div className="reg-grid">
                    ...
                    <div className="form-group full">
                      ...
                      <input type="text" className="form-control" value={name} onChange={e => setName(e.target.value)} placeholder="مثال: أحمد بوعلام" required={!isAnonymous} disabled={isAnonymous} />
                      <div className="checkbox-row" style={{ marginTop: '0.5rem', marginBottom: '0.5rem' }}>
                        ...
                        <label htmlFor="anon" style={{ fontSize: '0.9rem', color: 'var(--gray-600)' }}>عدم التصريح بالاسم (عرض كـ "فاعل خير")</label>
                      </div>
                    </div>

                    <div className="form-group full">
                      <label className="form-label"><Phone size={16}/> أوقات الاتصال المفضلة <span className="optional">(اختياري)</span></label>
                      <select className="form-control" value={contactTime} onChange={e => setContactTime(e.target.value)}>
                        <option value="">في أي وقت (الافتراضي)</option>
                        <option value="صباحاً">صباحاً (8:00 - 12:00)</option>
                        <option value="مساءً">مساءً (12:00 - 18:00)</option>
                        <option value="ليلاً">ليلاً (بعد 18:00)</option>
                      </select>
                    </div>
                    ...
                    <div className="form-group full">
                      <button type="submit" className="btn-submit" disabled={isSubmitting}>
                        <User size={20} /> {isSubmitting ? 'جاري التسجيل...' : 'سجّل الآن كمتبرع'}
                      </button>
                    </div>
                  </div>
                </form>
