    ...
    <div className={`donor-card-pro ${avail ? 'donor-available' : 'donor-unavailable'}`}>
      ...
      <div className="donor-card-body">
        <div className="donor-chips">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', width: '100%', paddingRight: '12px' }}>
            <span className="donor-chip chip-date" style={{ width: '100%', justifyContent: 'flex-start', padding: 0, gap: '10px', background: 'transparent', border: 'none' }}>
              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '16px' }}>
                <Calendar size={14} />
              </span>
              <span>آخر تبرع: منذ {daysSince} يوم</span>
            </span>
            {donor.preferred_contact && donor.preferred_contact !== 'any' && (
              <span className="donor-chip chip-contact" style={{ width: '100%', justifyContent: 'flex-start', padding: 0, gap: '10px', background: 'transparent', border: 'none' }}>
                <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '16px' }}>
                  <Clock size={14} />
                </span>
                <span>يفضل الاتصال: {donor.preferred_contact === 'morning' ? 'صباحاً (8:00 - 12:00)' : donor.preferred_contact === 'afternoon' ? 'مساءً (12:00 - 18:00)' : 'ليلاً (بعد 18:00)'}</span>
              </span>
            )}
          </div>
        </div>
      ...
      <div className="donor-card-footer-inline">
        ...
        <a href={`tel:${donor.phone}`} className="call-btn-inline">
          <span dir="ltr" style={{ direction: 'ltr', unicodeBidi: 'embed' }}>{donor.phone}</span>
          <Phone size={17} className="call-shake-icon" />
        </a>
      </div>
    </div>
  );
